...
Nota adicional para desarrolladores / CI:

- El script `npm test` en `backend` se ejecuta con `SKIP_DB_CHECK=true` para omitir la inicialización completa de la base de datos durante la suite de tests y evitar cargar binarios nativos (por ejemplo `sqlite3`) en entornos de CI o contenedores. Esto hace que los tests dependan de mocks/stubs que inyectan `config/database-init` cuando es necesario.
- Si quieres ejecutar la suite contra una base de datos real (por ejemplo con SQLite o Postgres configurado), usa:
