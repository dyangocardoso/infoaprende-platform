...
function validate(filePath) {
  const schemaPath = path.resolve(__dirname, '..', 'schemas', 'evaluacion.schema.json');
...
async function main() {
  ...
  if (!args[0]) {
    console.error('Uso: node scripts/validate-evaluacion.js <ruta-al-archivo-evaluacion>');
    process.exit(1);
  }
  const target = args[0];
